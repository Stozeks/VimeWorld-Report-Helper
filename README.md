...
<p align="center">
  <img src="docs/images/report-helper-main.png" alt="VimeWorld Report Helper — основной интерфейс" width="100%">
</p>
...
Если сканер не распознал нарушение, модератор может выделить нужное слово или фразу прямо в сообщении и нажать:

**🎓 Обучить сканер**

После этого можно указать, к какой категории относится выделенный текст.
...
можно связать с известным вариантом:

```text
шлюха
```

После подтверждения это знание сохраняется и может использоваться при следующих сканированиях.

Для записей учитываются дополнительные данные:
...
Обучение работает отдельно от официальной базы.

То есть расширение **не дописывает автоматически новые слова в `prohibited-words.txt`**.
